...
def check_inputs():
    ...
    if entry_items_number.get().isdigit():
        if int(entry_items_number.get()) < 0 or int(entry_items_number.get()) > 100:
            show_error_message("Total number of items hired must be a positive number and can not hire more than 100 items")
            input_check = 1
    else:
...
def setup_main_buttons():
    Button(main_window, text="Append Details",fg='blue',bg='lightblue',relief='ridge', bd=7 , command=open_add_details_page, width=15).grid(column=24, row=20, padx=22, pady=22)
    Button(main_window, text="Delete Details",fg='red',bg='lightblue',relief='ridge', bd=7 , command=open_delete_details_page, width=15).grid(column=24, row=21, padx=22, pady=22)
    Button(main_window, text="Print All Items Details",fg='darkgreen',bg='lightblue',relief='ridge', bd=7 , command=print_item_details, width=18).grid(column=26, row=21, padx=22, pady=22)
    Button(main_window, text="Search Hired Items",fg='brown',bg='lightblue',relief='ridge', bd=7, command=show_history, width=17).grid(column=24, row=22, padx=22, pady=22)
    Button(main_window, text="Quit",fg='yellow',bg='lightgreen',relief='ridge', bd=5 , command=quit_program, width=9).grid(column=27, row=20, padx=20, pady=20)
...
def open_add_details_page():
    ...
    append_image = PhotoImage(file="D:\PYTHON\我的电脑PYTHON\output-onlinepngtools (2).png")
    append_button = Button(add_details_window, text="Append Details", fg='black',bg='lightgreen', bd=5,command=check_inputs, width=185, image=append_image,compound="right")
    append_button.grid(column=1, row=3, padx=22, pady=22)
...
def main():
   ...
   # Load the background image
    bg_image = PhotoImage(file=r"output-onlinepngtools (3).png")  # Replace background image path
